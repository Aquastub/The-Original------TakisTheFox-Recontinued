/*	HEADER
	Everything achievement related goes here.
	Achievements are given out throughout the mod
...
rawset(_G,"TAKIS_ACHIEVEMENTINFO",{
	...
	[ACHIEVEMENT_RAKIS] = {
		...
		icon = "ACH_RAKIS",
		scale = FU/4,
		text = "Meet Rakis in Black Core 1 OR\n".."Change your color to Salmon",
		flags = 0,
	},
	...
	*/
	...
	[ACHIEVEMENT_METALSIGMA] = {
		name = "SILVER SIGMA",
		icon = "ACH_METALSIGMA",
		scale = FU/4,
		text = "Turn into the Metallic\nTakis Transformation.",
		flags = 0,
	},
	[ACHIEVEMENT_SPIRIT] = {
		name = "Spirits get!",
...
COM_AddCommand("sonadow", function(p, check, num)
	...
	if not p.takistable
		print("\x83TAKIS:\x80 Attempted to load achs for "..p.name.." without a valid takistable")
		return
	end
...
rawset(_G,"TakisAwardAchievement",function(p,achieve)
	...
	if modeattacking
		if TAKIS_DEBUGFLAG & DEBUG_ACH
			print("\x83TAKIS:\x80 "..p.name..": denied "..achinf[achieve].name.." because in modeattacking")
		end
		return
	end
	...
	if (number == nil)
		if TAKIS_DEBUGFLAG & DEBUG_ACH
			print("\x83TAKIS:\x80 "..p.name..": denied "..achinf[achieve].name.." because achfile is nil")
		end
		return
	end
	...
	if (TAKIS_TUTORIALSTAGE)
		if TAKIS_DEBUGFLAG & DEBUG_ACH
			print("\x83TAKIS:\x80 "..p.name..": denied "..achinf[achieve].name.." because in a tutorial stage")
		end
		return
	end
	...
	local trophy = P_SpawnMobjFromMobj(p.realmo,0,0,0,MT_RAY)
	trophy.state = S_TAKIS_TROPHY
	...
	for p2 in players.iterate
		...
		if p2.takistable.io.dontshowach == 1
			CONS_Printf(p2,"\x82"..p.name.." has just gotten the \x83"..TAKIS_ACHIEVEMENTINFO[achieve].name.."\x82 achievement!")
			continue
		end
	...
	if isdedicatedserver
		CONS_Printf(server,p.name.." has just gotten the "..TAKIS_ACHIEVEMENTINFO[achieve].name.." achievement!")
	end

	if discordtext
		if DiscordBot then
			DiscordBot.Data.msgsrb2 = $.."**"..p.name.."** has just gotten the ***"..TAKIS_ACHIEVEMENTINFO[achieve].name.."*** achievement!\n"
		end
	end
