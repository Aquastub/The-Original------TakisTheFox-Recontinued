/*	HEADER
	Handles everything Happy Hour related
...
rawset(_G,"HH_Trigger",function(actor,player,timelimit)
	if not hh.happyhour
		...
		for p in players.iterate
			...
			if (modeattacking)
			or (ultimatemode)
			or (p.starpostnum == TAKIS_NET.maxpostcount+32)
				starpoststuff = false
			end
			if starpoststuff
				--takis vars?
				...
				p.starpostnum = TAKIS_NET.maxpostcount+32
			...
			P_SetOrigin(p.realmo,
				actor.x,
				actor.y,
				GetActorZ(actor,actor,2) + (actor.scale*10*P_MobjFlip(actor))
			)
		end
...
rawset(_G,"HH_Reset",function()
	...
	hh.othergt = false --gametype == GT_PTSPICER
end)
...
local function CheckpointSequence(p,trig)
	if hh.happyhour then return end
	...
	if p.starpostnum == TAKIS_NET.maxpostcount+32
		...
		*/
		...
		p.exiting = 0
		me.momz = 0
		me.flags2 = $|(trig.flags2 & MF2_OBJECTFLIP)
...
addHook("ThinkFrame",do
	...
	if (gametype == GT_PTSPICER)
		if not (TAKIS_DEBUGFLAG & DEBUG_NOPTSRHH)
			hh.othergt = true
			...
			hh.gameovertics = PTSR.intermission_tics or PTSR.gameover_tics
			return
		else
			hh.othergt = true
		end
	...
	if hh.othergt
	and not (TAKIS_DEBUGFLAG & DEBUG_NOPTSRHH)
		--
	else
...
addHook("MobjThinker",function(trig)
	...
	--shitty
	if not (trig.spawedfromahh)
		CheckpointSequence(consoleplayer,trig)
	end
...
addHook("MobjCollide",function(trig,mo)
	...
	if P_MobjFlip(trig) == 1
		...
		*/
		...
		local midz = trig.z+(trig.height/2)
		--this should really be trig.z but stuff breaks otherwise
		local topz = trig.z+(trig.height)
	...
	if (hh.gameover)
		if (P_RandomChance(FU/(max(2,50-(hh.gameovertics/2)))))
			...
			local thok = P_SpawnMobjFromMobj(door,
				P_ReturnThrustX(nil,fa,range*door.scale)+P_RandomRange(-xvar,xvar)*door.scale,
				P_ReturnThrustY(nil,fa,range*door.scale)+P_RandomRange(-yvar,yvar)*door.scale,
...
addHook("MapLoad", function(mapid)
	...
	if (maptol & (TOL_NIGHTS|TOL_2D)) then return end
			...
			if (CV_TAKIS.happytime.value)
				for mt in mapthings.iterate
					...
					if mt.type == mobjinfo[MT_SIGN].doomednum
						trig = P_SpawnMobj(
							mt.x*FU + P_ReturnThrustX(nil,FixedAngle(mt.angle*FU),-10*FU), 
							mt.y*FU + P_ReturnThrustY(nil,FixedAngle(mt.angle*FU),-10*FU), 
							mt.z*FU,
							MT_HHTRIGGER
						)
