/*	HEADER
	Handles dropped heartcard mobjs
...
addHook("MobjThinker",function(card)
	...
	local dontspawnhere = false
	if (gametype == GT_ZE2)
	or not (G_CoopGametype()
	or (gametyperules & GTR_CAMPAIGN))
		dontspawnhere = true
	end
	
	if ((TAKIS_NET.cards == false or dontspawnhere) and card.spawnedfrommt ~= true) then P_RemoveMobj(card); return end
	...
	and P_RandomChance(FU*3/4)
		local rad,height = card.radius/card.scale, card.height/card.scale
		sparkle = P_SpawnMobjFromMobj(card,
			P_RandomRange(-rad,rad)*card.scale,
			P_RandomRange(-rad,rad)*card.scale,
			P_RandomRange(0,height)*card.scale,
			MT_BOXSPARKLE
		)
		sparkle.colorized = true
		sparkle.color = SKINCOLOR_ROSY
