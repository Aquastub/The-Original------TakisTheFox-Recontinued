/*	HEADER
	Handles shotgun mobj, bullets, and boxes
...
addHook("MobjThinker",function(mo)
	...
	if not mo.ragdoll
		...
		local angle = p.drawangle-ANGLE_90
		...
		P_MoveOrigin(mo,
			mo.target.x + P_ReturnThrustX(nil,angle,18*mo.scale),
			mo.target.y + P_ReturnThrustY(nil,angle,18*mo.scale),
			GetActorZ(mo.target,mo,1)
				+(mo.target.height/2*P_MobjFlip(mo.target))
		)
		...
		mo.pitch,mo.roll = mo.target.pitch,mo.target.roll
		mo.frame = A|trans
	else
		local rag = mo
		
		rag.pitch,rag.roll = 0,0
...
addHook("MobjThinker",function(shot)
	...
	if shot.timealive < 3 and hitscan then return end
...
--SHOTGUN BOXES
local SHOTGUN_GOLDEN = (1<<0)
local SHOTGUN_FORCED = (1<<1)

addHook("MapThingSpawn",function(mo,mt)
	if not (mo and mo.valid) then return end
	
	if mt.args[0] & SHOTGUN_GOLDEN
		mo.type = MT_SHOTGUN_GOLDBOX
	end
	if mt.args[0] & SHOTGUN_FORCED
		mo.forcebox = true
	end
