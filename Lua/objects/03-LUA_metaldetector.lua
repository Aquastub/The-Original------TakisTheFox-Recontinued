/*	HEADER
	Handles metal detector logic and 3d objects
...
addHook("TouchSpecial",function(door,mo)
	...
	if (p.mm and p.mm.weapon 
	and not p.mm.weapon.hidden 
	and p.mm.weapon.mobj 
	and p.mm.weapon.mobj.valid)
		hasit = true
	end
...
addHook("MobjThinker",function(door)
	...
	and not cullout
		...
		--spawn the splats
		list[1] = P_SpawnMobjFromMobj(door,0,0,0,MT_RAY)
		list[1].frame = A
		list[1].sprite = SPR_MTLD
		list[1].tics,list[1].fuse = -1,-1
		list[1].flags = TAKIS_3D_FLAGS|MF_NOTHINK
		...
		list[2] = P_SpawnMobjFromMobj(door,0,0,0,MT_RAY)
		list[2].frame = A
		list[2].sprite = SPR_MTLD
		list[2].frame = B
		list[2].tics,list[2].fuse = -1,-1
		list[2].flags = TAKIS_3D_FLAGS|MF_NOTHINK
		...
		for i = 1,4
			...
			list[2+i] = P_SpawnMobjFromMobj(door,
		...
		for i = 1,2
			local angle = door.angle+ANGLE_90
			local dist = (i == 1) and 30 or 24
			list[0+i] = P_SpawnMobjFromMobj(door,
				P_ReturnThrustX(nil,angle,dist*FU),
				P_ReturnThrustY(nil,angle,dist*FU),
				0,MT_RAY
			)
			list[0+i].frame = D
		...
		for i = 1,2
			local angle = door.angle+ANGLE_90
			angle = $+((i == 1) and ANGLE_90 or ANGLE_270)
		...
		local angle = door.angle+ANGLE_90
		list[5] = P_SpawnMobjFromMobj(door,
			P_ReturnThrustX(nil,angle,27*FU),
			P_ReturnThrustY(nil,angle,27*FU),
			0,MT_RAY
		)
		list[5].frame = F
		...
		list[5].angle = door.angle+ANGLE_90
		list[5].height = 0
		list[5].radius = 0
		P_SetOrigin(list[5],
			list[5].x,
		...
		list[1].angle = door.angle+ANGLE_90
		...
		door.alarm = {}
		list = door.alarm
		list[1] = P_SpawnMobjFromMobj(door,0,0,height,MT_RAY)
		list[1].frame = H
		list[1].sprite = SPR_MTLD
		list[1].tics,list[1].fuse = -1,-1
		list[1].flags = TAKIS_3D_FLAGS|MF_NOTHINK
